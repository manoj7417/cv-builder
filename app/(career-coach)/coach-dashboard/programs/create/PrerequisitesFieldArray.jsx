...
function PrerequisitesFieldArray({ control, register, errors, name }) {
    ...
    const [openAccordion, setOpenAccordion] = React.useState(prerequisiteFields.length > 0 ? 'item-0' : null);
    ...
    return (
        <div className='py-6'>
            ...
            <div className='py-4'>
                {prerequisiteFields.length > 0 && prerequisiteFields.map((prerequisite, index) => (
                    <Accordion type="single" collapsible className='border my-4 rounded-lg border-gray-200' key={index} value={openAccordion}>
                        <AccordionItem value={`item-${index}`}>
                            <AccordionTrigger className='w-full px-2 text-blue-800 py-4' onClick={() => handleAccordionToggle(`item-${index}`)}>
                                <div className='w-full flex items-center justify-between px-2'>
                                    <p>
                                        {prerequisites?.[index]?.type ? prerequisites[index].type : "(Not specified)"}
                                    </p>
                            ...
                            <AccordionContent>
                                <div className='py-2 px-8'>
                                    <div className='py-3'>
                                        <Label>Type<span className='text-red-500 ml-1'>*</span></Label>
                                        <Input
                                            placeholder='Enter type of prerequisite'
                                            className='my-2'
                                            {...register(`${name}.${index}.type`)}
                                        />
                                        <p className='text-red-500 text-sm ml-2'> {getError(name, index, 'type')}</p>
                                    </div>
                                    <div className='py-3'>
                                    ...
                                    <div className='py-3'>
                                        ...
                                        />
                                        <p className='text-red-500 text-sm ml-2'>{getError(name, index, 'attachmentUrl')}</p>
                                    </div>
                                </div>
                            </AccordionContent>
                        </AccordionItem>
            ...
            <div >
                <Button onClick={handleAddPrequisites} type='button'>Add Prerequisites<CirclePlus className='h-4 ml-1' /> </Button>
            </div>
        </div>
    );
}
