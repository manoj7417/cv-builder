...
function SubModuleFieldArray({ control, register, errors, name }) {
  ...
  const [openSubModuleAccordion, setOpenSubModuleAccordion] = useState(
    fields.length > 0 ? "item-0" : null
  );
  ...
  return (
    <div className="py-6">
      ...
      <div className="py-4">
        {fields.length > 0 &&
          fields.map((submodule, index) => (
            <Accordion
              type="single"
              collapsible
              className="border my-4 rounded-lg border-gray-200"
              key={index}
              value={openSubModuleAccordion}
            >
                ...
                <AccordionContent>
                  <div className="py-2 px-8">
                    ...
                    <div className="py-3">
                      ...
                      />
                      <p className="text-red-500 text-sm ml-2">
                        {getError(name, index, "timeToComplete")}
                      </p>
                    </div>
                  </div>
                </AccordionContent>
      ...
      <div className="py-4">
        <Button type="button" onClick={handleAddSubModule}>
          Add new submodule
          <CirclePlus className="h-4 ml-1" />
        </Button>
      </div>
      <p className="text-red-500 text-sm ml-2">
        {getSubModuleError() && `${getSubModuleError()}`}
      </p>
    </div>
  );
}
