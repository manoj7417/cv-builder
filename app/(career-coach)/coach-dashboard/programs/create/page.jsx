...
import { ChevronRight, CirclePlus, Trash2, Upload, LoaderCircle, ArrowLeft } from 'lucide-react';
...
function CreateProgram() {
    ...
    const { register, watch, setValue, handleSubmit, clearErrors, control, formState: { errors }, setError, reset } = useForm({
    ...
    const handleCreateProgram = async (data) => {
        ...
        try {
            ...
            data.days.forEach((day, dayIndex) => {
                const totalSubModulesTime = day.subModules.reduce((total, subModule) => total + (subModule.timeToComplete || 0), 0);
                if (totalSubModulesTime !== day.timeToComplete) {
                    isValid = false;
                    setError(`days.${dayIndex}`, {
                        type: 'manual',
                        message: 'Total time of sub-modules must match the time to complete for the day',
                    });
                }
            });
    ...
    return (
        <div className='w-full py-4'>
            <div className='px-8'>
                <TooltipProvider>
                    <Tooltip>
                        <TooltipTrigger><ArrowLeft className='cursor-pointer' onClick={() => router.back()} /></TooltipTrigger>
                        <TooltipContent>
                            <p>Go Back</p>
                        </TooltipContent>
                    </Tooltip>
                ...
                <form onSubmit={handleSubmit(handleCreateProgram)}>
                        ...
                        <Label>Program Image<span className='text-red-500 ml-1'>*</span></Label>
                        {
                            profileImage ?
                                <div className='flex items-center py-4'>
                                    <img src={profileImage} alt="profile image" className='w-52 h-52 object-cover rounded-lg' />
                                    <Button className='ml-4 bg-red-600 hover:bg-red-700' type='button' onClick={() => setValue("programImage", "")}>Remove</Button>
                                </div>
                                :
                                <div className='flex justify-around items-center'>
                                    <div className='w-2/5 flex justify-center items-center'>
                                        <Button onClick={handleOpenFileInput} disabled={isUploadingImage} type='button'>
                                            {isUploadingImage
                                                ? <>
                                                    Uploading <LoaderCircle className='h-4 ml-1 animate-spin' />
                                                </>
                                                :
                                                <>
                                        ...
                                        <Input type='file' className='my-2  hidden' {...register("programImage")} ref={fileInputRef} onChange={handleUploadImage} />
                                    </div>
                                    <p>OR</p>
                                    <Input {...register("programImage")} className='my-2 w-2/5' placeholder='Enter image url' />
                                </div>
                        }
                        <p className='text-red-500 text-sm ml-2'>{errors.programImage?.message}</p>
                    </div>
                    <div className='py-2'>
                        <Label>Program Video</Label>
                        {
                            programVideo ?
                                <div className='flex items-center py-4'>
                                    ...
                                    <Button className='ml-4 bg-red-600 hover:bg-red-700' type='button' onClick={() => setValue("programVideo", "")}>Remove</Button>
                                </div>
                                :
                    ...
                    <div className='w-full flex justify-end py-4'>
                        <Button type='submit' disabled={isCreatingProgram}>
                            {
                                isCreatingProgram ? <>
                                    Creating Program <LoaderCircle className='h-4 ml-1 animate-spin' /></>
                                    :
