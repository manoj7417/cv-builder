    ...
    try {
      ...
      if (data.status === 'success' && data.data) {
        ...
        if (data.data.pagination) {
          ...
          setTotalJobs(data.data.pagination.totalDocs || data.data.total || sortedJobs.length)
        } else {
      ...
      <div className="bg-[#0d3572] text-white py-20 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <p className="text-xl text-gray-200 max-w-2xl animate-fade-in-delay">
            Discover opportunities that match your skills and aspirations. Browse through {totalJobs} open positions.
          </p>
        </div>
      </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-gray-600 mt-1">{job.recruiter?.company}</p>
                    </div>
                    
                    <div className="flex flex-wrap gap-2">
                      <span className={`px-3 py-1.5 text-sm font-medium rounded-full inline-flex items-center ${
                        job.type === 'Full-time' ? 'bg-green-100 text-green-800 border border-green-200' :
                        job.type === 'Part-time' ? 'bg-blue-100 text-blue-800 border border-blue-200' :
                        job.type === 'Contract' ? 'bg-yellow-100 text-yellow-800 border border-yellow-200' :
                      ...
                      <span className={`px-3 py-1.5 text-sm font-medium rounded-full inline-flex items-center ${
                        job.location.toLowerCase().includes('remote') ? 'bg-green-100 text-green-800 border border-green-200' :
                        job.location.toLowerCase().includes('hybrid') ? 'bg-blue-100 text-blue-800 border border-blue-200' :
                  ...
                  <Link
                    ...
                    className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-[#0d3572] hover:bg-[#0d3572]/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#0d3572] transition-all duration-300 hover:scale-105"
                  >
                    Apply Now
                  </Link>
                </div>
