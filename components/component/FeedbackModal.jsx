...
function FeedbackModal({ content, showModal, onClick, onClose }) {
  return (
    <div
      className={`absolute 2xl:w-[400px] lg:w-[350px] right-10 rounded-lg  p-6 shadow-xl bg-white z-40 ${
            ...
            content.map((item, index) => {
              return (
                <li key={index} className=" text-xs flex items-start">
                  <span className=" flex justify-center items-center rounded-full mr-3 font-bold">
                    {index + 1}.
                  </span>
                  {item}
                </li>
